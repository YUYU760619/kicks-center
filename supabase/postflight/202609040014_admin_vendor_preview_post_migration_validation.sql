-- Read-only postflight for migration 014. Expected healthy result: one PASS row.
...
issues(issue_type, record_id, detail) as (
  ...
  union all
  select
    'SHARED_BUILDER_FORBIDDEN_PRODUCT_FIELD',
    'private.kc_build_vendor_portal_snapshot(text)',
    'Product note/location must not be emitted'
  from function_catalog
  where schema_name = 'private'
    and function_name = 'kc_build_vendor_portal_snapshot'
    and function_oid is not null
...
select
  'PASS',
  'kc_pos_state/main',
  '0 issues; migration 014 installed and vendor preview integrity verified'
