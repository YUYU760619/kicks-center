-- Read-only postflight for migration 016. Expected healthy result: one PASS row.
with expected_functions(
  ...
  values
    ...
    ('private', 'kc_guard_sale_vendor_visibility', '', true, 'v', false),
    ('public', 'kc_admin_set_sale_vendor_visibility',
      'text, boolean, timestamp with time zone', true, 'v', true)
),
function_catalog as (
  select
    e.*,
...
required_helpers(schema_name, function_name, argument_types) as (
  values
    ...
    ('private', 'kc_assert_pos_financial_integrity', 'jsonb'),
    ('private', 'kc_assert_stage1_core', 'jsonb')
),
helper_catalog as (
  select
    e.*,
...
expected_triggers(tgname, table_schema, table_name, function_schema, function_name) as (
  values
    ('kc_pos_state_00_vendor_code_guard_trigger',
      'public', 'kc_pos_state', 'private', 'kc_apply_vendor_code_snapshots'),
    ('kc_pos_state_01_sale_vendor_visibility_guard',
      'public', 'kc_pos_state', 'private', 'kc_guard_sale_vendor_visibility')
),
trigger_status as (
  select
    e.*,
    t.oid trigger_oid,
    t.tgenabled,
    actual_n.nspname actual_table_schema,
    c.relname actual_table_name,
    actual_pn.nspname actual_function_schema,
    p.proname actual_function_name
  from expected_triggers e
  left join pg_catalog.pg_trigger t on t.tgname = e.tgname and not t.tgisinternal
  left join pg_catalog.pg_class c on c.oid = t.tgrelid
  left join pg_catalog.pg_namespace actual_n on actual_n.oid = c.relnamespace
  left join pg_catalog.pg_proc p on p.oid = t.tgfoid
  left join pg_catalog.pg_namespace actual_pn on actual_pn.oid = p.pronamespace
),
...
issues(issue_type, record_id, detail) as (
  ...
  select
    ...
    tgname,
    'Expected exactly one trigger with this name; count=' || count(trigger_oid)::text
  from trigger_status
  group by tgname
  having count(trigger_oid) <> 1

  union all
  select
    'TRIGGER_TARGET_INVALID',
    tgname,
    coalesce(actual_table_schema, '(missing)') || '.' || coalesce(actual_table_name, '(missing)')
      || ' -> ' || coalesce(actual_function_schema, '(missing)') || '.'
      || coalesce(actual_function_name, '(missing)')
  from trigger_status
  where trigger_oid is not null
    and (
      actual_table_schema is distinct from table_schema
      or actual_table_name is distinct from table_name
      or actual_function_schema is distinct from function_schema
  ...
  select
    'TRIGGER_DISABLED',
    tgname,
    'tgenabled=' || coalesce(tgenabled::text, 'NULL')
  from trigger_status
  where trigger_oid is not null
  ...
  where source_name not in ('public.kc_pos_state', 'jsonb_array_elements', 'jsonb_array_elements_text')
  ...
  union all
  select
    'VISIBILITY_GUARD_CONTRACT_INVALID',
    'private.kc_guard_sale_vendor_visibility()',
    'Guard must default new Sales, reject malformed booleans, and block non-dedicated changes'
  from function_catalog
  where schema_name = 'private'
    and function_name = 'kc_guard_sale_vendor_visibility'
    and function_oid is not null
    and (
      position('jsonb_typeof(v_sale -> ''vendorvisible'') <> ''boolean''' in normalized_definition) = 0
      or position('jsonb_build_object(''vendorvisible'', true)' in normalized_definition) = 0
      or position('else true' in normalized_definition) = 0
      or position('kc.allow_sale_vendor_visibility_change' in normalized_definition) = 0
      or position('sale_vendor_visibility_requires_dedicated_admin_operation' in normalized_definition) = 0
    )

  union all
  select
    'ADMIN_VISIBILITY_MUTATION_CONTRACT_INVALID',
    'public.kc_admin_set_sale_vendor_visibility(text, boolean, timestamptz)',
    'Mutation must authorize Admin, lock/version state, change only the Sale field, and audit'
  from function_catalog
  where schema_name = 'public'
    and function_name = 'kc_admin_set_sale_vendor_visibility'
    and function_oid is not null
    and (
      position('if not private.kc_is_admin() then' in normalized_definition) = 0
      or position('for update' in normalized_definition) = 0
      or position('v_current_updated_at is distinct from p_expected_updated_at' in normalized_definition) = 0
      or position('sale ->> ''sale_id''' in normalized_definition) = 0
      or position('jsonb_build_object(''vendorvisible'', p_vendor_visible)' in normalized_definition) = 0
      or position('kc.allow_sale_vendor_visibility_change' in normalized_definition) = 0
      or position('kc.action_summary' in normalized_definition) = 0
      or position('jsonb_set(v_payload, ''{sales}''' in normalized_definition) = 0
    )

  union all
  select
    'BUILDER_VISIBILITY_PROJECTION_INVALID',
    'private.kc_build_vendor_portal_snapshot(text)',
    'Hidden Product/Sale and legacy visibility projection contract is incomplete'
  from function_catalog
  where schema_name = 'private'
    and function_name = 'kc_build_vendor_portal_snapshot'
    and function_oid is not null
    and (
      position('hidden_sale ->> ''productid'' = item ->> ''id''' in normalized_definition) = 0
      or position('when not (hidden_sale ? ''vendorvisible'') then false' in normalized_definition) = 0
      or position('then to_jsonb(''在庫''::text) else item -> ''status'' end' in normalized_definition) = 0
      or position('when not (sale ? ''vendorvisible'') then true' in normalized_definition) = 0
      or position('when jsonb_typeof(sale -> ''vendorvisible'') = ''boolean''' in normalized_definition) = 0
      or position('else false' in normalized_definition) = 0
    )

  union all
  select
    'BUILDER_SETTLEMENT_PROJECTION_INVALID',
    'private.kc_build_vendor_portal_snapshot(text)',
    'Settlement must expose only visible saleIds, recompute payout, and omit hidden-only records'
  from function_catalog
  where schema_name = 'private'
    and function_name = 'kc_build_vendor_portal_snapshot'
    and function_oid is not null
    and (
      position('''saleids'', projected.visible_sale_ids' in normalized_definition) = 0
      or position('''payout'', projected.visible_payout' in normalized_definition) = 0
      or position('sum(coalesce(nullif(sale ->> ''return_price'', '''')::numeric, nullif(sale ->> ''cost'', '''')::numeric))' in normalized_definition) = 0
      or position('jsonb_array_length(projected.visible_sale_ids) > 0' in normalized_definition) = 0
    )
  ...
  select
    ...
    'private.kc_build_vendor_portal_snapshot(text)',
    'Projected status and Settlement output keys must be exact'
  from function_catalog
  where schema_name = 'private'
    and function_name = 'kc_build_vendor_portal_snapshot'
    and function_oid is not null
    and (
      position('''status'', case when exists' in normalized_definition) = 0
      or position('''id'', projected.settlement -> ''id''' in normalized_definition) = 0
      or position('''completedat'', projected.settlement -> ''completedat''' in normalized_definition) = 0
    )
  ...
  union all
  select
    'SALE_VENDOR_VISIBILITY_MALFORMED',
    coalesce(sale ->> 'sale_id', sale ->> 'id', '(missing)'),
    'vendorVisible must be absent for legacy compatibility or contain a JSON boolean'
  from sales
  where sale ? 'vendorVisible'
    and jsonb_typeof(sale -> 'vendorVisible') is distinct from 'boolean'
)
...
select
  'PASS',
  'kc_pos_state/main',
  '0 issues; migration 016 installed and visibility/privacy contracts verified'
