...
create or replace function private.kc_vendor_portal_snapshot()
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  return private.kc_build_vendor_portal_snapshot(v_vendor_id);
end
$$;

revoke all on function private.kc_vendor_portal_snapshot() from public, anon;
grant execute on function private.kc_vendor_portal_snapshot() to authenticated;

create or replace function public.kc_admin_get_vendor_preview(p_vendor_id text)
returns jsonb
language plpgsql
stable
security definer
...
revoke all on function public.kc_admin_get_vendor_preview(text)
from public, anon, authenticated;
grant execute on function public.kc_admin_get_vendor_preview(text)
to authenticated;
