  ...
  return <div className="min-h-screen bg-[#11151a]">
    ...
    <main className="lg:pl-[246px]">
      ...
      <div className="mx-auto max-w-[1500px] p-4 sm:p-7">
        <div className="mb-6 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div className="flex items-center gap-4">{storedVendor && <Link href={`/vendors/${storedVendor.id}/preview`} aria-label={`預覽 ${storedVendor.name} 的供應商頁面`} className="grid min-h-16 min-w-24 place-items-center rounded-2xl border border-[#e8893a]/35 bg-[#e8893a]/10 px-3 py-2 text-center text-[10px] font-black leading-4 text-[#e8893a] transition hover:-translate-y-0.5 hover:border-[#e8893a] hover:bg-[#e8893a]/15 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#e8893a]">預覽<br/>供應商頁面<br/>→</Link>}<div><div className="mb-1 flex items-center gap-2"><span className="font-mono text-xs font-black text-[#e8893a]">{activeVendorCodes.map((code) => code.code).join(' / ')}</span><span className="rounded-full border border-[#5daa83]/25 bg-[#5daa83]/10 px-2 py-0.5 text-[9px] font-bold text-[#74bb96]">合作中</span></div><h1 className="text-2xl font-black sm:text-3xl">{vendor.name}</h1><p className="mt-1 text-xs text-zinc-500">統一管理商品、異動、售出與結款紀錄</p></div></div><Link href="/?page=inbound" className="rounded-xl bg-[#e8893a] px-4 py-3 text-center text-xs font-black text-[#17120e]">＋ 新增此廠商商品</Link></div>
