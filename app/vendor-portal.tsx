...
export function VendorPortal() {
  ...
  return (
    <div className="min-h-screen bg-[#11151a] text-[#e7eaee]">
      ...
      <main className="mx-auto max-w-[1400px] p-4 sm:p-7">
        ...
        <section className="mt-5">
          <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex gap-2 overflow-x-auto">{tabs.map((name) => <button key={name} onClick={() => setTab(name)} className={`whitespace-nowrap rounded-full px-4 py-2 text-xs font-bold ${tab === name ? "bg-[#e8893a] text-[#17120e]" : "border border-[#3a4552] text-[#98a2ad]"}`}>{name}</button>)}</div>
