  ...
  return <>
    <main className="mx-auto max-w-[1400px] p-4 sm:p-7">
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[
        ...
        ["已售未結", unsettled.length, "件"], ["待結款", money(unsettled.reduce((sum, sale) => sum + sale.cost, 0)), ""],
      ...
      <section className="mt-5">
        ...
        <div className="kc-card overflow-x-auto"><table className="kc-table"><thead><tr>{["貨號", "商品", "尺寸", "型號", "應收回價", "寄賣開始", "狀態"].map((label) => <th key={label}>{label}</th>)}</tr></thead><tbody>{filtered.map((product) => <tr key={product.id} onClick={() => setSelected(product)} className="cursor-pointer"><td className="font-mono font-bold text-orange-400">{product.code}</td><td><b>{product.name}</b><div className="mt-1 text-[10px] text-zinc-600">{product.brand} · {product.color}</div></td><td>US {product.usSize}<div className="text-[10px] text-zinc-600">{product.cmSize} CM</div></td><td className="text-zinc-400">{product.model}</td><td>{money(product.cost)}</td><td>{product.consignmentStart}</td><td><span className={`rounded-full border px-2.5 py-1 text-[10px] font-bold ${statusClass[product.status] || statusClass.已下架}`}>{product.status}</span></td></tr>)}</tbody></table>{!filtered.length && <div className="py-16 text-center text-xs text-zinc-600">此分類目前沒有商品</div>}</div>
      </section>
      <section className="mt-5 grid gap-5 lg:grid-cols-2">
        <div className="kc-card p-6"><h2 className="text-sm font-black">最近售出</h2><div className="mt-5 space-y-4">{sales.slice().sort((a, b) => b.soldAt.localeCompare(a.soldAt)).slice(0, 6).map((sale) => { const product = products.find((item) => item.id === sale.productId); return <div key={sale.id} className="flex items-center justify-between gap-4 border-b border-[#303944] pb-4 last:border-0 last:pb-0"><div><div className="text-xs font-bold">{product?.name || sale.productId}</div><div className="mt-1 text-[10px] text-zinc-600">{date(sale.soldAt)}</div></div><div className={`text-xs font-bold ${sale.settled ? "text-[#6f91c9]" : "text-[#d9a441]"}`}>{sale.settled ? "已銷帳" : "待結款"}</div></div>; })}{!sales.length && <div className="text-xs text-zinc-600">尚無銷售紀錄</div>}</div></div>
    ...
    {selected && <div className="fixed inset-0 z-50 flex justify-end bg-black/70" onClick={() => setSelected(null)}><aside className="h-full w-full max-w-[480px] overflow-y-auto border-l border-[#303944] bg-[#1d232b] p-6" onClick={(event) => event.stopPropagation()}><div className="flex items-center justify-between"><div><div className="font-mono text-xs font-bold text-[#e8893a]">{selected.code}</div><h2 className="mt-1 text-xl font-black">商品資料</h2></div><button onClick={() => setSelected(null)} className="grid h-9 w-9 place-items-center rounded-full bg-white/5 text-zinc-400">×</button></div><div className="mt-6 rounded-2xl bg-[#171c22] p-5"><div className="text-lg font-black">{selected.name}</div><div className="mt-2 text-xs text-zinc-500">{selected.brand} · {selected.model}</div></div><div className="mt-6 grid grid-cols-2 gap-4">{[["尺寸", `US ${selected.usSize} / ${selected.cmSize} CM`], ["應收回價", money(selected.cost)], ["包裝", selected.packaging], ["寄賣開始", selected.consignmentStart], ["狀態", selected.status]].map(([label, value]) => <div key={label}><div className="text-[9px] text-zinc-600">{label}</div><div className="mt-1 text-xs font-bold">{value}</div></div>)}</div></aside></div>}
  </>;
}
